...
export default {
  ...
  methods: {
    ...
    modifyUser(row) {
      userId = row.userId;
      req('getUserInfoById',{...this.userId}).then((data) =>{
        if(data.code === 1){
    ...
    saveUserForm() {
      ...
      if (this.dialogInfo.type === "modifyUser") {
        if (this.userForm.identity === "student") {
          ...
          req('getStudentInfo',{...this.sId}).then((data) =>{
            if(data.code === 1){
        ...
        if (
          ...
          this.dialogInfo.title = "修改";
          req('getTeacherId',{...this.teacherId}).then((data) => {
            if(data.code === 1){
    ...
    saveStudentForm() {
      ...
      if (this.dialogInfo.type === "modifyStudent") {
        ...
        //   }
        // });
        req("modifyStudent", { ...this.studentForm,...this.userForm }).then((data) => {
          if (data.code === 1) {
            this.fetch();
            this.$refs[this.studentForm].resetFields();
            this.$refs[this.userForm].resetFields();
    ...
    saveTeacherForm() {
      if (this.dialogInfo.type === "addTeacher") {
        ...
        //   }
        // });
        req("insertTeacher", { ...this.teacherForm,...this.userForm }).then((data) => {
          if (data.code === 1) {
            this.fetch();
      ...
      if (this.dialogInfo.type === "modifyTeacher") {
        ...
        //   }
        // });
        req("modifyTeacher", { ...this.teacherForm,...this.userForm }).then((data) => {
          if (data.code === 1) {
            this.fetch();
            this.$refs[this.teacherForm].resetFields();
            this.$refs[this.userForm].resetFields();
