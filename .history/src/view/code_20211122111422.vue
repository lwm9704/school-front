    ...
    <el-dialog :title="dialogInfo.title" :visible.sync="codeFormVisible">
      <el-form :model="codeForm" :rules="codeRules" :ref="codeForm">
        ...
        <el-form-item label="编码描述" :label-width="formLabelWidth">
          <el-input v-model="detailForm.description" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  data() {
    return {
      ...
      detailRules: {
        value: [{ required: true, message: "请输入编码值", trigger: "blur" }],
  ...
  methods: {
    ...
    saveCodeForm() {
      this.$refs[this.codeForm].validate((valid) => {
        // console.log(this.$refs[this.codeForm])\
