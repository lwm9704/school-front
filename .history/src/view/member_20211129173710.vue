<template>
  <div>
    <div class="main-head" :model="search">
      ...
      <dl>
        <dd>
          <el-select v-model="search.classesId" clearable placeholder="班级">
            <el-option
    ...
    <el-dialog :title="dialogInfo.title" :visible.sync="memberFormVisible">
      <el-form :model="memberForm" :rules="memberRules" :ref="memberForm">
        ...
        <el-form-item label="学生名称" prop="sId" :label-width="formLabelWidth">
  ...
  data() {
    return {
      ...
      memberRules: {
        ...
        sId: [{ required: true, message: "请输入学生姓名", trigger: "blur" }],
      ...
      this.$refs[this.memberForm].validate((valid) => {
        if (valid) {
          ...
          if (this.dialogInfo.type === "editMember") {
            if(this.memberForm.type === '0'){
              this.memberForm.type = '外公';
            }
            if(this.memberForm.type === '1'){
              this.memberForm.type = '外婆';
            }
            if(this.memberForm.type === '2'){
              this.memberForm.type = '爷爷';
            }
            if(this.memberForm.type === '3'){
              this.memberForm.type = '奶奶';
            }
            if(this.memberForm.sex === '0'){
              this.memberForm.sex = '男';
            }
            if(this.memberForm.sex === '1'){
              this.memberForm.sex = '女';
            }
