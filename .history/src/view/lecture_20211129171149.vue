<template>
  <div>
    <div class="main-head" :model="search">
      ...
      <dl>
        <dd>
          <el-select v-model="search.courseId" placeholder="课程名称">
            <el-option
...
export default {
  ...
  methods: {
    ...
    editLecture(row) {
      ...
      this.lectureFormVisible = true;
      this.$nextTick(() => {
        this.lectureForm = JSON.parse(JSON.stringify(row));
      });
    },
      ...
      this.$refs[this.lectureForm].validate((valid) => {
        if (valid) {
          ...
          if (this.dialogInfo.type === "editLecture") {
            req("updateData", { ...this.lectureForm }).then((data) => {
              console.log(data.data);
              if (data.code === 1) {
                this.$message.success(data.msg);
                this.fetch();
