<template>
  <div>
    <div class="main-head" :model="search">
      ...
      <dl>
        <dd>
          <el-select v-model="search.identity" clearable placeholder="请选择">
            <el-option
      ...
      <el-dialog title="学生信息" :visible.sync="studentFormVisible">
        <el-form :model="studentForm" :rules="studentRules" :ref="studentForm">
          ...
          <el-form-item
            ...
            :label-width="formLabelWidth"
          >
            <el-select v-model="studentForm.classesId" placeholder="请选择">
              <el-option
        ...
        <div slot="footer" class="dialog-footer">
          <!--待修改补充-->
          <el-button @click="cancelStudentForm">取 消</el-button>
      ...
      <el-dialog title="老师信息" :visible.sync="teacherFormVisible">
        <el-form :model="teacherForm" :rules="teacherRules" :ref="teacherForm">
          ...
          <el-form-item
            ...
            :label-width="formLabelWidth"
          >
            <el-input v-model="teacherForm.idCar" autocomplete="off"></el-input>
          </el-form-item>
          <el-form-item
            label="毕业学校"
        ...
        <div slot="footer" class="dialog-footer">
          <el-button @click="cancelTeacherForm">取 消</el-button>
...
export default {
  data() {
    return {
      ...
      classList: [],
      studentRules: {
        sId: [{ required: true, message: "请选择", trigger: "blur" }],
        ...
        mName: [{ required: true, message: "请输入", trigger: "blur" }],
        moPhone: [
  ...
  methods: {
      ...
      if (row.identity === "student") {
        ...
        req("getStudentInfoById", { id: this.id }).then((data) => {
          if (data.code === 1) {
            console.log(data.data);
            this.$nextTick(() => {
              data.data.sId = data.data.sid;
              data.data.fName = data.data.fname;
              data.data.fPhone = data.data.fphone;
              data.data.mName = data.data.mname;
              data.data.mPhone = data.data.mphone;
              data.data.sName = data.data.sname;
              data.data.cPhone = data.data.cphone;
              data.data.hAddress = data.data.haddress;
              data.data.hState = data.data.hstate;
              this.studentForm = data.data;

              if (this.studentForm.sex === "男") {
                this.studentForm.sex = 0;
              }
              if (this.studentForm.sex === "女") {
      ...
      if (
        ...
        req("getTeacherInfoById", { id: this.id }).then((data) => {
          if (data.code === 1) {
            console.log(data.data);
            this.$nextTick(() => {
              data.data.tName = data.data.tname;
              data.data.tPhone = data.data.tphone;
    ...
    saveStudentForm() {
      ...
      if (this.dialogInfo.type === "modifyStudent") {
        ...
        //   }
        // });
        this.studentForm.studentId = this.id;
        req("updateStudent", { ...this.studentForm }).then((data) => {
          if (data.code === 1) {
            this.fetch();
            this.$refs[this.studentForm].resetFields();
            this.userForm = { brand_right: 0 };
    ...
    deletedUser(row) {
      this.$confirm("此操作将永久删除,是否继续?").then(() => {
        ...
        req("deleteUser", { userId: userId }).then((data) => {
          if (data.code === 1) {
            if (identity === "student") {
