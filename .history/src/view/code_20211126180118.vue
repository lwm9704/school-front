<template>
  <div>
    <div class="main-head" :model="search">
      ...
      <dl>
        <dd>
          <el-select v-model="search.state" clearable placeholder="编码值状态">
            <el-option
              v-for="item in state"
              :key="item.value"
              :label="item.label"
    ...
    >
      ...
      <el-table-column label="编码" prop="codeNo" width="150"></el-table-column>
      <el-table-column
        label="编码描述"
      ...
      <el-table-column label="操作" width="600">
        <template slot-scope="scope">
          <el-button size="mini" type="primary" @click="modifyCode(scope.row)"
            >编码编辑</el-button
          >
          <el-button size="mini" type="primary" @click="addDetail(scope.row)"
            >新增编码详情</el-button
          >
          <el-button size="mini" type="primary" @click="modifyDetail(scope.row)"
            >修改编码详情</el-button
          >
          <el-button size="mini" @click="deleteDetail(scope.row)"
            >删除编码详情</el-button
          >
          <el-button size="mini" @click="deleteCode(scope.row)"
            >删除编码</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="dialogInfo.title" :visible.sync="codeFormVisible">
      <el-form :model="codeForm" :rules="codeRules" :ref="codeForm">
        <el-form-item label="编码" prop="codeNo" :label-width="formLabelWidth">
    ...
    <el-dialog :title="dialogInfo.title" :visible.sync="detailFormVisible">
      <el-form :model="detailForm" :rules="detailRules" :ref="detailForm">
        ...
        >
          <el-input
            v-model="detailForm.codeValue"
            autocomplete="off"
          ></el-input>
        </el-form-item>
        <el-form-item
          label="编码值描述"
          prop="codeValueDescribe"
          :label-width="formLabelWidth"
        >
          <el-input
            v-model="detailForm.codeValueDescribe"
            autocomplete="off"
          ></el-input>
        </el-form-item>
...
export default {
  data() {
    return {
      ...
      codeRules: {
        ...
        description: [{ required: true, message: "请输入编码描述", trigger: "blur" }],
      },
      //详情表单
      ...
      detailRules: {
        ...
        codeValueDescribe:[{ required: true, message: "请输入编码值描述", trigger: "blur" }],
  ...
  methods: {
    ...
    modifyCode(row) {
      this.codeForm.codeId = row.codeId;
      this.codeForm.codeNo = row.codeNo;
      this.codeForm.description = row.codeDescription;
    ...
    modifyDetail(row) {
      this.detailForm.codeId = row.codeId;
      this.detailForm.codeDetailedId = row.codeDetailedId;
      this.detailForm.codeValue = row.codeValue;
      this.detailForm.codeValueDescribe = row.codeValueDescription;
      ...
      this.$refs[this.codeForm].validate((valid) => {
        if (valid) {
          ...
          if (this.dialogInfo.type === "modifyCode") {
            req("updateData", { ...this.codeForm }).then((data) => {
              console.log(data.data);
              if (data.code === 1) {
      ...
      this.$refs[this.detailForm].validate((valid) => {
        if (valid) {
          ...
          if (this.dialogInfo.type === "modifyDetail") {
            ...
            req("updateDetailData", { ...this.detailForm }).then((data) => {
              console.log(data.data);
              if (data.code === 1) {
                this.$message.success(data.msg);
                this.fetch();
    ...
    deleteCode(row) {
      this.$confirm("此操作将永久删除该文件,是否继续?").then(() => {
        let codeId = row.codeId;
        let codeDetailedId = row.codeDetailedId;
        if (codeDetailedId != null) {
          req("deleteDetailDataById", { codeDetailedId: codeDetailedId }).then(
            (data) => {
              if (data.code === 1) {
                this.$message.success(data.msg);
                this.fetch();
        ...
        req("deleteData", { codeId: codeId }).then((data) => {
          if (data.code == 1) {
            this.fetch();
          } else {
