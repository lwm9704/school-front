    ...
    >
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button size="mini" type="primary" @click="scoreEdit(scope.row)"
            >修改信息</el-button
          >
    ...
    <el-dialog :title="dialogInfo.title" :visible.sync="scoreFormVisible">
      <el-form :model="scoreForm" :rules="scoreRules" :ref="scoreForm">
        ...
        <el-form-item label="学生" prop="sId" :label-width="formLabelWidth">
          <el-select v-model="scoreForm.sId" clearable placeholder="请选择">
            <el-option
  ...
  data() {
    return {
      ...
      scoreRules: {
        ...
        sId:[{ required: true, message: "请选择学生", trigger: "blur" },],
      ...
      this.$refs[this.scoreForm].validate((valid) => {
        if (valid) {
          ...
          if (this.dialogInfo.type === "scoreEdit") {
            if(this.scoreForm.state === '发布'){
              this.scoreForm.state = 1;
            }
            if(this.scoreForm.state === '下线'){
              this.scoreForm.state = 0;
            }
            if(this.scoreForm.semester === '第一个学期'){
              this.scoreForm.semester = 1;
            }
            if(this.scoreForm.semester === '第二个学期'){
              this.scoreForm.semester === 2;
            }
