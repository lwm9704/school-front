  ...
  <div>
    ...
    <el-dialog :title="dialogInfo.title" :visible.sync="codeFormVisible">
      <el-form :model="codeForm" :rules="codeRules" :ref="codeForm">
        ...
        <el-form-item label="编码描述" prop="description" :label-width="formLabelWidth">
    ...
    <el-dialog :title="dialogInfo.title" :visible.sync="detailFormVisible">
      <el-form :model="detailForm" :rules="detailRules" :ref="detailForm">
        <el-form-item label="编码值" prop="codeValue" :label-width="formLabelWidth">
          <el-input v-model="detailForm.codeValue" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="编码值描述" prop="codeValueDescribe" :label-width="formLabelWidth">
  ...
  methods: {
    ...
    deleteDetail(row) {
      this.$confirm("此操作将永久删除该文件,是否继续?").then(() => {
        let codeDetailId = row.codeDetailId;
        req("deleteDetailDataById", { codeDetailId: codeDetailId }).then((data) => {
          this.$message.success(data.data.msg);
          this.fetch();
        });
      });
