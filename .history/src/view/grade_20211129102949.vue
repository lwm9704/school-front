  ...
  <div>
    ...
    >
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button size="mini" type="primary" @click="headmasterEdit(scope.row)"
            >安排班主任</el-button
          >
          <el-button size="mini" type="primary" @click="teacheringEdit(scope.row)"
            >安排任课老师</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="pageInfo.title" :visible.sync="gradeFormVisible">
      <el-form :model="gradeForm" :rules="gradeRules" :ref="gradeForm">
        ...
        <el-form-item label="班级人数" prop="gradeNumber" :label-width="formLabelWidth">
          <el-input v-model="gradeForm.classesNumber" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="班主任" prop="headmasterId" :label-width="formLabelWidth">
          <el-select
...
export default {
  ...
  methods: {
    ...
    headmasterEdit(row) {
      ...
      this.gradeFormVisible = true;
      this.$$nextTick(() => {
        utils.copyFormObject(data,this.gradeForm);
      });
      this.gradeForm = row;
    },
    teacheringEdit(row) {
