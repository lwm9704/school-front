    ...
    >
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="primary" @click="teachingEdit(scope.row)">修改信息</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    teachingEdit(row) {
      ...
      this.teachingForm.teacherId = row.teacherName;
      this.teachingForm.map(function (val){
        if(val.semester === '第一个学期'){
          val.semester = 1
        }
        if(val.semester === '第二个学期'){
          val.semester = 2
        }
      })
